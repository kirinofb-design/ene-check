...
export function countFusionClientBatches(rangeStart: string, rangeEnd: string): number {
  ...
  if (isVercelHostedClient()) {
    ...
    return FUSION_SOLAR_STATIONS.length * Math.max(1, periodSlices);
  }
...
export function estimateProdFullCollectMinutes(
  ...
  const perFusionBatchMin = shouldUseFusionDayWindowClient() ? 2 : isVercelHostedClient() ? 4 : 8;
  const perFusionBatchMax = shouldUseFusionDayWindowClient() ? 4 : isVercelHostedClient() ? 8 : 18;
  return {
...
export function formatFusionOnlyTimeHint(rangeStart: string, rangeEnd: string): string | null {
  ...
  const min = Math.max(20, Math.round(chunks * 4));
  const max = Math.max(min + 10, Math.round(chunks * 8));
